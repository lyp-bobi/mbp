...
import org.apache.spark.memory.{MemoryMode, UnifiedMemoryManager,StaticMemoryManager}
...
import org.apache.spark.util.{ByteBufferInputStream, ByteBufferOutputStream, SizeEstimator}
...
class mbpMemoryStoreSuite extends FunSuite with BeforeAndAfter with BeforeAndAfterEach with PrivateMethodTester {
...
/*
  ...
  test("safely unroll blocks through putIteratorAsValues") {
    ...
    putIteratorAsValues("b5", smallIterator, ClassTag.Any)
    assert(memoryStore.contains("b1"))
    assert(!memoryStore.contains("b2"))
